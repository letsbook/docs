...
function compressVideo(filePath) {
    ...
    try {
        ...
        if (result.error || result.status !== 0) {
            console.error(
                `  ❌ Failed to compress: ${result.stderr.toString()}`
            );
        ...
        console.log(
            `  ✅ Compressed: ${originalMB}MB → ${compressedMB}MB (${savings}% smaller)`
        );
        return true;
    } catch (error) {
...
function main() {
    ...
    for (const file of mp4Files) {
        if (isAlreadyCompressed(file)) {
            console.log(
                `⏭️  Skipping (already compressed): ${path.relative(DOCS_DIR, file)}`
            );
            skipped++;
        } else {
