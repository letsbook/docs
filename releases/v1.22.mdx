...
- **Set margins per slot.** Give each slot its own buffer instead of one setting for the whole schedule. A multi-day charter can get plenty of changeover time while a quick two-hour trip gets just a little, and you can add that time before a slot, after it, or both. Great for cleaning, prep, inspection, or a late return. [Set it in your slot schedules](/guides/settings/rental-setups/schedules/slot-schedule#buffer-time).
