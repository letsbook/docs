...
import downPaymentVideo from '../graphics/settings_confirmation_downpayment.mp4';
import payToConfirmVideo from '../graphics/settings_confirmations_pay_to_confirm.mp4';
...
<InlineVideoPlayer videoSrc={payToConfirmVideo} title="Set opening times" />
...
<InlineVideoPlayer videoSrc={downPaymentVideo} />

### How it works

Customer completes booking → pays down payment → booking confirmed → you collect remaining balance before the trip.

**Collecting the remaining balance:** Open the booking in your dashboard, go to the Finance tab, and send a payment link. The customer receives an email with a secure link to complete their payment. Learn more about [managing booking finances](/guides/day-to-day/bookings/manage-booking-finances).
...
Learn more about how all these settings work together in [Understanding rental setup](/guides/settings/rental-setups).
