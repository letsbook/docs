...
<Button href="https://dashboard.letsbook.app/pricing/slots/add">
    Create slot pricing
</Button>

## Prerequisites

Create a [slot schedule](/guides/settings/rental-setups/schedules/slot-schedule) first. Slot pricing connects to existing schedules and sets rates for each time slot.
...
<InlineVideoPlayer videoSrc={settingsPricingSlotsVideo} />
...
- **Display only at the payment step:** Only show this charge when the customer reaches payment, not earlier in the booking form
- **Do not display the cost if it amounts to exactly 0:** Hide the charge completely when it calculates to $0. Keeps the payment overview clean for your customers

**Example:** Charge $5.00 per passenger, added on top of the base slot price.

<InlineVideoPlayer videoSrc={pricingAdditionalChargeVideo} />
...
<InlineVideoPlayer videoSrc={pricingDepositVideo} />
