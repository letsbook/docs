...
import notificationsEditVideo from '../graphics/notifications_edit.mp4';
...
<InlineVideoPlayer videoSrc={notificationsEditVideo} />
...
Automatically sent when booking details change (time, date, boat, etc.). Add the Booking.Changes variable to show exactly what changed, old and new side by side. Prefer these not to go out at all? Turn them off for your whole account in your [general settings](https://dashboard.letsbook.app/general-info).
...
**Email 24 hours before pickup**
Optional reminder sent one day before the trip. Enable this in your notification flow to give customers a heads-up before their trip.
...
- Partner code (included automatically in tenant-facing confirmation, update, and cancellation emails when a booking has one)
...
## See what a customer received

Every notification sent for a booking is logged on that booking's Notifications tab, including delivery status and a one-click resend. See [track customer notifications](/guides/day-to-day/bookings/booking-notifications).
