...
- **Unit** (default) - Fixed price per item.
- **Boat** - Multiplies by the number of boats in the booking.
- **Passenger** - Multiplies by the number of passengers.
- **Hour per boat (exact time)** - A 1h45 booking is charged for 1.75 hours, per boat.
- **Day per boat (exact time)** - A 26-hour booking is charged for 1.08 days, per boat.
- **Day per boat (rounded up)** - A 26-hour booking is charged for 2 days, per boat.
- **Passenger per hour** - Multiplies by passengers × hours.
