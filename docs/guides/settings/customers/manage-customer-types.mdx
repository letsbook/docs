...
**Color** - Give the type its own color so bookings and planning are easier to read at a glance. Open the customer type, pick a color, and you're set.
...
**Exempt from signing waivers** - Skip waiver requests entirely for this customer type. Your regulars signed once when they joined, so they don't need to sign again on every trip. Bookings from other customer types keep asking as usual.
