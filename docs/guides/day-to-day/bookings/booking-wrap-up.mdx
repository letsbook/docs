...
Next to the dedicated page, the [bookings overview](https://dashboard.letsbook.app/bookings) shows a Wrap-up button beside the date scope, with the number of bookings waiting. Your dashboard shows the same count for the last 14 days, and says All wrapped up when you're done.

## Three tabs

- **To do**: bookings where money still has to move: a balance that isn't settled, or a security deposit still to return even when the balance is already paid. Opens on Last 3 days, with a 4–14 days ago pill next to it. Anything older lives under Custom, where you pick a return date range or All.
