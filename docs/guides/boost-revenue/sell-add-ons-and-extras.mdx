...
**Per-passenger-per-hour quantity caps:** Perfect for consumables like drinks. "2 beers per person per hour" automatically scales the limit to group size and trip length. Prevents over/under ordering.
...
**Match price to scale:** Configure pricing methods so the math actually scales with the booking. Per passenger for drinks, per boat for BBQ kits, per hour per boat for heaters or captain service. Set this on each add-on in [add-on settings](/guides/settings/create-add-ons).
...
Bookings with add-on orders show a shopping cart icon on the booking bar in your [planning overview](https://dashboard.letsbook.app/planning). Hover the booking to see exactly which items were ordered. Filter by date, dock, boat model, category, or product. Print order lists for staff. Export summaries for supplier reconciliation.
